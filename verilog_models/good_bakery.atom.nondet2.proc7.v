...
module bakery(clock,select,pause,sym_break,mutation);
    ...
    case (pc[sel])
        ...
        for (k = 0; k <= HIPROC; k = k + 1)
            ...
            if (/*j[sel] <= k &&*/ !break)
                ...
                if (choosing[k])
                    ...
                    defSel = defer[sel];
                    defK = defer[k];
                    defSelK = defSel[k]; //defSelK = extract(defSel,k,mutation);
                    defKSel = extract(defK,sel,mutation); //defKSel = defK[sel];
        ...
        for (i = 0; i <= HIPROC; i = i + 1) begin
            defK = defer[i];
            defer[i] = clearBit(defK,sel,mutation);
        end
        pc[sel] = L11;
        end
